...
接入后，请 **QQ 群主**根据需要开启“获取群内全部消息”和“机器人主动在群聊内发言”。**这两项权限仅群主可设置，群管理员不能代为操作**；如果你不是群主，请联系群主完成。接入本身使用收到的消息进行被动回复，不要求事先开启主动推送。
...
不同 QQ 版本的界面可能不同，图片仅作位置参考。没有对应设置入口时，请先确认正在使用群主账号；群主仍找不到入口时，再尝试更新 QQ。相关权限未开放时，不应将接入成功理解为主动推送已可用。
